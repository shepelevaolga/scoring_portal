...
Scenario: 1.01 Sign in page is available for user
    Given user is on the page with "Sign in with Google" button
...
Scenario: 1.08 User tries to sign in without saved Google account
...
Scenario: 1.09 User tries to sign in with non-existing email
    Given user sees "Email or phone" input
    When user fills in "Email or phone" input with <non-existing email>
    Examples:
        | non-existing email |
        | ifmnvme@gmail.com  |
...
Scenario: 1.10 User tries to sign in with non-existing phone number
    Given user sees "Email or phone" input
    When user fills in "Email or phone" input with <non-existing phone>
    Examples:
        | non-existing phone |
        | +79819139699       |
    And user clicks "Next"
...
Scenario: 1.11 User tries to sign in with incorrect password
    ...
    And user clicks "Next"
    When user fills in "Enter your password" input with <incorrect password>
    Examples:
        | incorrect password |
        | Trewqasdf4321?     |
    And user clicks "Next"
...
Scenario: 1.15 User tries to log in with invalid data
    Given user sees "Email or phone" input
    When user fills in "Email or phone" input with <invalid data>
    Examples:
        | invalid data |
        | +73455829    |
        | kmegjbmp@rfg |
...
Scenario: 1.16 Username is saved
    Given user is successfully signed in
    And user changed his username in Quantori Scoring to <username>
    Examples:
        | username    |
        | Dmitry      |
        | JohnDoe1337 |
    And user signed out
    When user signs in with the same account
    Then user sees saved <username> on the Quantori Scoring main page