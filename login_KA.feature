Feature: log in Scoring 
Background: 
    Given installed and launched VPN on local PC
    And a Gmail account
...
Scenario: 1_1.1 Log in with valid login and password which were saved
   Given user has login and password for gmail account saved on the device
...
Scenario: 1_1.2 Log in with valid login and password
   Given user has login and password for gmail account 
...
Scenario: 1_2.1 Log in with invalid password
    ...
    When user fills a valid login in "login field"
...
Scenario: 1_2.2 Log in with invalid login
    Given user has a google account
    When user fills a invalid login in "login field"
    And user clicks "<next> button"
...
Scenario: 1_4.1 Log in using another account, which is not google one
    ...
    And clicks "<next> button"
    Then user seer the error message "google account is not finded" and "login field" marked red
...
Scenario: 1_5.1 Log in with empty login
    ...
    When user clicks "<change the account> button"
    And clicks "<next> button"
    Then user seer the error message "enter email address or phone number" and "login field" marked red
...
Scenario: 1_5.2 Log in with empty password
    ...
    And clicks "<next> button"
    Then user seer the error message "enter password" and "password field" marked red
...
Scenario: 1_7.1 User log in with 2f authorisation on
    Given user has a google account
    When user clicks "<change the account> button"
    And fills the valid login
    And clicks "<next> button"
    And fills the valid password
    And clicks "<next>"
